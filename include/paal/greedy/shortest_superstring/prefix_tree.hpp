...
public:
    prefix_tree(int length,
               ...
               std::vector<int> const& lengthSuffixWord) :
                 m_length(length),
                 m_prefix_tree(m_length),
                 m_which_son_am_i(m_length),
                 m_prefix_to_tree(m_length),
                 m_suffix_to_tree(m_length),
                 m_suffix_array(suffix_array),
                 m_sum_words(sumWords),
                 m_lcp(lcp),
    ...
    void erase_word_form_prefix_tree(int wordBegin){
        for(int letterOfWord=0;m_sum_words[letterOfWord+wordBegin]!=DELIMITER;++letterOfWord) {
    ...
    void fill_suffix_to_tree(){
        for(int suffix=m_length-1,lastWord=0,commonPrefix=0;suffix>0;suffix--){
            ...
            if(beginOfSuffix == 0 || m_sum_words[beginOfSuffix - 1] == DELIMITER) {
                lastWord = beginOfSuffix;
                commonPrefix = m_lcp[suffix];
            }
            else{
                if(commonPrefix == m_length_suffix_word[beginOfSuffix]){
                    m_suffix_to_tree[suffix]=m_prefix_to_tree[lastWord+commonPrefix-1];
                }
