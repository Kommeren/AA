...
knapsack_general_two_app(
    ...
    std::vector<ObjectsIter> objects(boost::make_counting_iterator(oBegin),
    ...
    auto greedyFill = get_greedy_fill(objects.begin(), objects.end(), capacity,
    ...
    auto largest =
        std::max_element(objects.begin(), objects.end(),
                         utils::make_functor_to_comparator(starValue));
