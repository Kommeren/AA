...
public:
    ...
    steiner_tree(const Metric & m, const voronoi & vor) :
        m_metric(m), m_voronoi(vor), N(std::distance(vor.get_generators().begin(), vor.get_generators().end())),
            m_save(N)  {}
    ...
    void get_result_steiner_vertices(OutputIterator out) {
        ...
        auto ng = [&](const AMatrix & t) {
            return boost::make_iterator_range(boost::make_zip_iterator(boost::make_tuple(subsets.first,
                                        boost::make_transform_iterator(m_subs_dists.begin(), utils::remove_reference()))),
                                  boost::make_zip_iterator(boost::make_tuple(subsets.second,
                                        boost::make_transform_iterator(m_subs_dists.end(), utils::remove_reference()))));
        };
        ...
        auto lsSolution = data_structures::metric_to_bgl_with_index(
                        m_metric,
                        m_voronoi.get_generators().begin(),
                        m_voronoi.get_generators().end(),
                        m_t_idx);
...
private:
    ...
    typedef std::unordered_map<ThreeTuple, VertexType, boost::hash<ThreeTuple>> NearstByThreeSubsets;
        ...
        template <typename Iter> std::pair<data_structures::subsets_iterator<SUSBSET_SIZE, Iter>,
    ...
    void fill_sub_dists() {
        ...
        for(const ThreeTuple & subset : boost::make_iterator_range(subRange)) {
            ...
            auto vRange1 =  m_voronoi.get_vertices_for_generator(m_t_idx.get_val(std::get<0>(subset)));
            auto vRange2 =  m_voronoi.get_vertices_for_generator(m_t_idx.get_val(std::get<1>(subset)));
            auto vRange3 =  m_voronoi.get_vertices_for_generator(m_t_idx.get_val(std::get<2>(subset)));
            auto range = boost::join(boost::join(vRange1, vRange2), vRange3);

            if(boost::empty(range)) {
                m_nearest_vertex[subset] = *m_voronoi.get_vertices().begin();
            } else {
                m_nearest_vertex[subset] =  *std::min_element(boost::begin(range), boost::end(range),
                        utils::make_functor_to_comparator([&](VertexType v){return this->dist(v, subset);}));
            }
            m_subs_dists.push_back(this->dist(m_nearest_vertex[subset], subset));
        }
    }
    ...
    void move_save(const SpanningTree & G1, const SpanningTree & G2, Dist maxDist) {
        auto v1 = vertices(G1);
        auto v2 = vertices(G2);
