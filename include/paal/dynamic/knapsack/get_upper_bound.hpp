...
template <typename ObjectsIter, typename ObjectSizeFunctor,
          typename ObjectValueFunctor, typename Is_0_1_Tag>
...
get_value_upper_bound(ObjectsIter oBegin, ObjectsIter oEnd,
                      ...
                      Nonarithmetic_size_tag, Is_0_1_Tag) {
...
get_value_upper_bound(ObjectsIter oBegin, ObjectsIter oEnd,
                      FunctorOnIteratorPValue<ObjectSizeFunctor, ObjectsIter>
                          capacity, // capacity is of size type
                      ObjectValueFunctor value, ObjectSizeFunctor size,
                      arithmetic_size_tag, Is_0_1_Tag is_0_1_Tag) {
    auto out = boost::make_function_output_iterator(utils::skip_functor());

    return std::min(2 * knapsack_general_two_app(oBegin, oEnd, capacity, out,
                                                 value, size, is_0_1_Tag).first,
                    get_density_based_value_upper_bound(oBegin, oEnd, capacity,
                                                        value, size));
}
...
get_value_upper_bound(ObjectsIter oBegin, ObjectsIter oEnd,
    ...
    return get_value_upper_bound(oBegin, oEnd, capacity, value, size,
                                 Getarithmetic_size_tag<SizeType>(),
                                 is_0_1_Tag);
}
