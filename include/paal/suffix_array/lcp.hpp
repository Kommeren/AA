...
void lcp(std::vector<int> const& suffix_array,std::vector<int> const&rank,std::vector<int> &lcp,
    ...
    for(auto suffixRank: rank){//suffixRank number suffix in lexicographically order
        if(suffixRank!=0){
            while(sumWords[suffix_array[suffixRank]+comonPrefixLength]==sumWords[suffix_array[suffixRank-1]+comonPrefixLength]){
                ++comonPrefixLength;
            }
        }
