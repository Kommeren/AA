...
BOOST_AUTO_TEST_CASE(zelikovsky_11_per_6_test) {
    ...
    for (paal::steiner_tree_test& test : data) {
        ...
        voronoiT voronoi(FSet(test.terminals.begin(), test.terminals.end()),
                         FSet(test.steiner_points.begin(), test.steiner_points.end()), test.metric);
        std::vector<int> selectedSteinerPoints;
        paal::steiner_tree::steiner_tree_zelikovsky11per6approximation(test.metric, voronoi, std::back_inserter(selectedSteinerPoints));
        ...
        auto g = paal::data_structures::metric_to_bgl_with_index(test.metric, boost::begin(resRange), boost::end(resRange), idx);
