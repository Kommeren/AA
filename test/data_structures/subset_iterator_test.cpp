...
BOOST_AUTO_TEST_CASE(subset_iterator_engine_test) {
    auto engine = paal::data_structures::make_subsets_iterator_engine<3>(
        v.begin(), v.begin() + 4);
...
BOOST_AUTO_TEST_CASE(subset_iterator_test) {
    ...
    BOOST_CHECK_EQUAL(std::get<0>(*std::begin(r)), 1);
    BOOST_CHECK_EQUAL(std::get<1>(*std::begin(r)), 2);
    BOOST_CHECK_EQUAL(std::get<2>(*std::begin(r)), 3);
