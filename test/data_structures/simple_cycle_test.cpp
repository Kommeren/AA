...
class check_size {
public:
    check_size(simple_cycle<string> cm, std::size_t size) : m_cm(cm), m_size(size) {}
...
void check_swap_edge(T::iter b, T::iter e, const El & p1, const  El & p2, const El & start, const Sol & sol) {
...
template <typename T>
struct simpl_cycle_temp : public simple_cycle<T> {
    template <typename Iter> simpl_cycle_temp(Iter b, Iter e) : simple_cycle<T>(b, e) {}
};
