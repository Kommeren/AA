...
BOOST_AUTO_TEST_CASE(componentsTest) {
    ...
    BOOST_CHECK_EQUAL(comps.get<names::A>(),
                      0); // we guarantee that POD's are value initialized
    ...
    static_assert(
        ds::detail::has_template_get<Comps<int, double, int>, names::A>::value,
        "HasGet doesn't work");
...
struct X {
    ...
    template <typename ostream> friend ostream &operator<<(ostream &o, X x) {
        o << x.x;
        return o;
    }
