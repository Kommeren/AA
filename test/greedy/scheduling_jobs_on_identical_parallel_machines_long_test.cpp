    ...
    for (int numberOfMachines = min_machines; numberOfMachines <= max_machines;
        ...
        for (double minJobsOnMachine = min_jobs_on_machine_start;
            ...
            std::vector<Time> jobs = generate_job_loads(
                machines, minJobsOnMachine, max_time,
            ...
            paal::greedy::scheduling_jobs_on_identical_parallel_machines(
                numberOfMachines, jobs.begin(), jobs.end(),
            ...
            Time sumAllLoads =
                std::accumulate(sumOfMachine.begin(), sumOfMachine.end(), 0.0);
            // print result
            check_result(double(maximumLoad),
                         double(sumAllLoads) / numberOfMachines, 4.0 / 3.0);
        }
    }
}
