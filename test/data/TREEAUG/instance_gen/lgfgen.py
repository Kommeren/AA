#Tree augmentation instance generator in python language
# (generates tree+cycle instances) Taken from
#http://www.imada.sdu.dk/~marco/e12aug/
#http://www.imada.sdu.dk/~marco/e12aug/TC-gen.py
...
# d+1 is an upper bound on the degree of any vertex in T
    ...
    #print Edges
    fd = open(out,"w")
    #fd.write("@nodes"+str(n)+" "+str(e)+'\n')
    fd.write("@nodes  "+str(n)+'\n')
    fd.write("label"+'\n')
    for c in range(n):
	fd.write(str(c)+"\n")

    fd.write("@edges   "+str(e)+'\n')
    fd.write("  label intree cost"+'\n')
    #fd.write("#tree edges"+'\n')
	...
	f=Edges[c]	
        if c<e-l:
            E=str(f[0])+" "+str(f[1])+" "+str(c)+" 1 0"
	else:
	    E=str(f[0])+" "+str(f[1])+" "+str(c)+" 0 "+str(f[2])
	fd.write(E+"\n")
    fd.close()
    #print 'ciao'
    return
...
for x in range (2,10):
    random.seed(x)
    t(800,x,0,"lgfinst/out"+str(x)+".lgf")
#
#
