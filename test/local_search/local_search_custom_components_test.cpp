...
BOOST_AUTO_TEST_CASE(conditional_gain_adaptor_test) {
    ...
    auto condGain = ls::make_conditional_gain_adaptor(
        search_comps().get<ls::Gain>(), [](int sol, int move) {
        return sol + move <= 3;
    });

    ls::first_improving(solution, paal::data_structures::replace<ls::Gain>(
                                      condGain, search_comps()));
...
BOOST_AUTO_TEST_CASE(tabu_gain_adaptor_test) {
    ...
    ls::first_improving(currentSolution,
                        ls::make_search_components(get_moves(), condGain,
