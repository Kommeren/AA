...
struct Virtualget_moves {
    virtual auto operator()(
        ls::n_queens_solution_adapter<std::vector<int>> &sol)
        ->decltype(ls::n_queensget_moves{
    }(sol)) {
        return ls::n_queensget_moves{}
        (sol);
    }
};
...
struct virtual_commit {
    virtual bool operator()(
        ls::n_queens_solution_adapter<std::vector<int>> &sol, ls::Move m) {
        return ls::n_queens_commit{}
        (sol, m);
    }
};
...
int main(int argc, char **argv) {
    ...
    auto countingComps = paal::local_search::make_search_components(
        getMoves, countingGain, commit);
