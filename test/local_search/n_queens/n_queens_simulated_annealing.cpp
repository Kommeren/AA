...
template <typename F> struct random_moves {
    random_moves(F _f) : f(_f) {}
    typedef paal::local_search::Move result_type;

    result_type operator()() const { return f(); }
...
int main(int argc, char **argv) {
    ...
    auto commit = [](Adapter & sol, ls::Move m) {
        return n_queens_commit_sa()(sol, m.get_from(), m.get_to());
    };
    auto randomMove = [ = ]() mutable {
        return ls::Move(distribution(rand), distribution(rand));
    };
    ...
    auto countingGain =
        paal::utils::make_counting_functor_adaptor(gain, nr_of_iterations);

    // alternative cooling strategy
    /// auto coolingStrategy =
    /// ls::make_exponential_cooling_schema_dependant_on_time(std::chrono::seconds(10),
    /// 100, 0.1);
    auto coolingStrategy =
        ls::exponential_cooling_schema_dependant_on_iteration(10, 0.9999, 1000);
    auto saGain = ls::make_simulated_annealing_gain_adaptor(countingGain,
                                                            coolingStrategy);
