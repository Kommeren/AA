...
template <typename Metric, typename Cost> class fl_logger {
  public:
    fl_logger(const Metric &m, const Cost &c) : m_metric(m), m_cost(c) {}

    template <typename Sol> bool operator()(Sol &sol) {
        ON_LOG(auto const &ch =
                   sol.getfacility_location_solution().get_chosen_facilities());
        LOG_COPY_RANGE_DEL(ch, ",");
        ON_LOG(
            auto c =
                sol.getfacility_location_solution().get_voronoi().get_cost());
        LOGLN("current cost "
              << simple_algo::get_cfl_cost(m_metric, m_cost,
                                           sol.getfacility_location_solution())
...
fl_logger<Metric, Cost> make_fl_logger(const Metric &m, const Cost &c) {
...
void run_tests(const std::string &fname, Solve solve) {
    ...
    while (is_test_cases.good()) {
        ...
        VorType voronoi(FSet{ fac.begin(), fac.end() },
                        VSet(clients.begin(), clients.end()), metric,
                        facCapacities, verticesDemands);
        Sol sol(std::move(voronoi), USet{}, cost);

        solve.template operator()<VorType>(sol, metric, cost, opt,
                                           make_fl_logger(metric, cost));
...
struct solve_add_remove {
    ...
    void operator()(Solution &sol, const Metric &metric, Cost cost, double opt,
                    Action a) {
        ...
        LOGLN(std::setprecision(20)
              << "BEFORE SWAP APPROXIMATION RATIO: " << c / opt);
    }
};
